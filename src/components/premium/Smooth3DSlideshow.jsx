...
const DEFAULT_SLIDES = [
    {
        iframe: { src: "/template-saas.html" },
        title: "SaaS Analytics\nDashboard",
    },
    {
        iframe: { src: "/template-ecommerce.html" },
        title: "E-Commerce\nStorefront",
    },
    {
        iframe: { src: "/template-portfolio.html" },
        title: "Creative\nPortfolio",
    },
    {
        iframe: { src: "/template-saas.html" },
                ...
                >
                    {list.map((slide, i) => {
                        ...
                        return (
                            ...
                            >
                                {slide._id ? (
                                    <div className="group bg-white rounded-xl flex flex-col h-full w-full overflow-hidden text-left relative pointer-events-auto">
                                        <div className="aspect-[4/3] w-full relative bg-slate-100 overflow-hidden border-b border-slate-100">
                                            <div className="absolute inset-0 bg-slate-900/5 group-hover:bg-transparent transition-colors duration-300 z-10 pointer-events-none" />
                                            <img
                                                src={slide.imageUrl}
                                                alt={slide.title}
                                                loading="lazy"
                                        ...
                                        <div className="p-5 flex flex-col flex-1">
                                            <div className="mb-2 pointer-events-none">
                                                <h4 className="text-lg font-bold text-black leading-tight">{slide.title}</h4>
                                            </div>
                                            <p className="text-black text-xs mb-4 h-8 line-clamp-2 pointer-events-none" title={slide.shortDescription}>
                                                {slide.shortDescription}
                                            </p>
                                            <div className="grid grid-cols-2 gap-2 mt-auto pt-4 border-t border-slate-100 relative z-30">
                                                <Link
                                                    to={`/template/${slide._id}`}
                                                    onClick={(e) => !isActive && e.preventDefault()}
                                                    className="flex items-center justify-center gap-1 py-2 rounded-xl bg-slate-50 text-black font-semibold text-xs hover:bg-slate-100 transition-colors border border-slate-200"
                                                >
                                                    <ExternalLink size={14} />
                                                    Details
                                                </Link>
                                                <a
                                                    href={`https://wa.me/919598037255?text=${encodeURIComponent(`Hi ToolBite! 👋\n\nI'm interested in purchasing the *${slide.title}* template.\n\n📦 *Category:* ${slide.category}\n\nCould you please guide me through the next steps?`)}`}
                                                    target="_blank"
                                                    rel="noopener noreferrer"
                                                    onClick={(e) => !isActive && e.preventDefault()}
                                                    className="flex items-center justify-center gap-1 py-2 rounded-xl bg-black text-white font-semibold text-xs hover:bg-black/90 transition-all shadow-md"
                                                >
