...
const TestimonialCard = ({ review, index, containerRef }) => {
  ...
  return (
    ...
    >
      <div className={`bg-white border border-slate-200 shadow-sm ${currentStyle.shadow} hover:border-transparent hover:bg-gradient-to-br ${currentStyle.bg} transition-all duration-500 p-6 rounded-2xl flex flex-col h-full relative overflow-hidden z-10`}>
        <div className="flex gap-1 mb-4 relative z-10">
          {[...Array(review.rating)].map((_, idx) => (
            <Star key={idx} size={14} fill="currentColor" className="text-amber-400 drop-shadow-sm" />
          ))}
        </div>
        <p className="text-slate-600 group-hover:text-white/95 text-sm leading-relaxed mb-6 italic flex-1 relative z-10 transition-colors duration-300">
          "{review.text}"
        </p>
        <div className="flex items-center gap-3 mt-auto pt-4 border-t border-slate-50 group-hover:border-white/20 relative z-10 transition-colors duration-300">
          ...
          <div>
            <h4 className="font-bold text-slate-900 group-hover:text-white text-sm leading-tight transition-colors duration-300">{review.name}</h4>
...
const Testimonials = () => {
  ...
  return (
    <section id="testimonials" className="py-12 bg-slate-50">
